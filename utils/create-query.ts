export function create_query(values,req,res,regex?) {
    return new Promise((resolve, reject) => {
        ...
        if (values.query_type == 'or' || values.query_type == 'and') {
            ...
            try {type = eval(values.query_type);} catch (error) {}
            if (!type) {
                if (!type) {
                    if (req.method == 'POST') type = req.body[values.query_type];
                    if (req.method == 'GET') type = req.query[values.query_type];
                }
                if (type != 'or' && type != 'and') {
                    type = 'and';
                }
        ...
        for (const key in values.query_values) {
            ...
            try {test_value = eval(value_key);} catch (error) {}
            if (!test_value) {
                if (req.method == 'POST') test_value = req.body[value_key];
                if (req.method == 'GET') test_value = req.query[value_key];
            }
            let qdata = {};
            if (regex) {
